...
data_files = [
    'orders.csv', 'order_details.csv', 'products.csv', 
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            ...
            kinesis.put_record(
                ...
                PartitionKey=str(random.randint(1, 10)) # TODO : Use a better partitioning strategy based on the stream you're going create
            )
            ...
            if len(batch) >= BATCH_SIZE:  # Adjust batch size as needed
...
def write_batch_to_s3(entity, batch):
    ...
    # Write as Parquet with Snappy compression to memory
    buffer = BytesIO()
    pq.write_table(
        table, 
        buffer,
        compression='snappy' # Use Snappy compression for Parquet
    )

    # Create date-partitioned S3 key (e.g. raw/users/year=2025/month=03/day=21/...)
    now = datetime.now()
    s3_key = (
        f'{S3_PREFIX}/{entity}'
        f'year={now.year}/month={now.month:02d}/day={now.day:02d}/'
        f'{uuid4().hex}.parquet'  # Unique filename using UUID'
    )
